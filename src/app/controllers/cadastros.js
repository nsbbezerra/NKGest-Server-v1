...
router.post('/clientes', async (req, res) => {
    ...
    const { name, cpf_cnpj, rg, emitter, dateBirth, email, typeClient, socialName, stateRegistration, municipalRegistration, manager, phoneComercial, celOne, celTwo, obs, comissioned } = req.body;
    ...
    if(findName) {
        return res.status(400).send({ message: 'Já existe um cliente com este nome cadastrado' });
    }
    
    if(findCpf) {
        return res.status(400).send({ message: 'Já existe um CPF/CNPJ cadastrado' });
    }
...
router.post('/enderecos', async (req, res) => {
    ...
    const { client ,street, number, comp, bairro, cep, city, state, obs } = req.body;
...
router.post('/fornecedores', async (req, res) => {
    ...
    const { name, cpf_cnpj, rg, emitter, dateBirth, email, typeClient, socialName, stateRegistration, typeTaxPayer, municipalRegistration, manager, phoneComercial, celOne, celTwo, obs, cep, city, state } = req.body;
    ...
    if(findName) {
        return res.status(400).send({ message: 'Já existe um fornecedor com este nome cadastrado' });
    }
...
router.post('/funcionarios', async (req, res) => {
    ...
    const { name, gender, dateBirth, celOne, celTwo, email, admin, sales, caixa, admission, comission, user, password, cargo, comissioned } = req.body;
    ...
    if(findName) {
        return res.status(400).send({ message: 'Já existe um funcionário com este nome cadastrado' });
    }

    if(funcionario){
        return res.status(400).send({ message: 'Este funcionário já está cadastrado' });
    }
...
router.post('/produtos', async (req, res) => {
    ...
    const { name, fornecedor, code, codeUniversal, unMedida, description, valueCusto, valueDiversos, valueSale, estoqueAct, active, cfop, ncm, icms, pis, cofins, cest, margeLucro, markupFactor } = req.body;
...
router.post('/servicos', async (req, res) => {
    ...
    if(findName) {
        return res.status(400).send({ message: 'Já existe um serviço com este nome cadastrado' });
    }

  ...
  try {
    ...
    if (find === 1) {
      const vendas = await Vendas.find({
        ...
        year: yearDate,
        $or: [{ rascunhoNFE: { $exists: false } }, { rascunhoNFE: false }],
      }).populate({
...
router.post("/nfeRasc", async (req, res) => {
  ...
  try {
    ...
    await Vendas.findByIdAndUpdate(sale._id, {
      $set: { rascunhoNFCE: true, nfce: true },
    });
...
router.post("/nfe", async (req, res) => {
  ...
  try {
    ...
    const nfeRas = await NFCE.findOneAndUpdate(
      { sale: id },
      {
        $set: {
          natureza_operacao,
    ...
    if (statusResponse === "processando_autorizacao") {
      ...
      return res.status(200).send({
        message:
          "Aguarde alguns instantes, a NFCE está sendo processada pela SEFAZ",
        info,
      });
    }
    ...
    if (statusResponse === "denegado") {
      ...
      return res.status(200).send({
        message:
          "Houve uma inconsistência nos dados cadastrais, a NFCE foi denegada pela SEFAZ",
        info,
      });
    }
...
router.delete("/delRasc/:id", async (req, res) => {
  ...
  try {
    await NFCE.findOneAndRemove({ sale: id });

    await Vendas.findByIdAndUpdate(id, {
      $set: { rascunhoNFCE: false, nfceStatus: "" },
    });
...
router.post("/statusNfe", async (req, res) => {
  ...
  try {
    ...
    const rasc = await NFCE.findOne({ sale: ref });
    ...
    let urlNfe = `${NfeConfig.url}/v2/nfce/${referencia}?completa=(1)`;
    ...
    if (info === "processando_autorizacao") {
      await Vendas.findByIdAndUpdate(ref, { $set: { nfceStatus: info } });
      return res.status(200).send({
        message:
          "Aguarde alguns instantes, a NFCE está sendo processada pela SEFAZ",
        info,
    ...
    if (info === "autorizado") {
      ...
      await Vendas.findByIdAndUpdate(ref, {
        $set: { nfceStatus: info, nfeDanfeUrl: danfeUrl, nfeXmlUrl: xmlUrl },
      });
      return res
        .status(200)
        .send({ message: "NFCE Autorizada", info, danfeUrl, xmlUrl });
    }
    ...
    if (info === "cancelado") {
      ...
      let messageSafaz = response.mensagem_sefaz;
      await Vendas.findByIdAndUpdate(ref, {
        $set: { nfceStatus: info, xmlCancUrl: xmlCancUrlNFe },
      });
      return res.status(200).send({
        message: "Esta NFCE foi cancelada.",
        statuSefaz,
        messageSafaz,
        info,
      });
...
router.post("/cancelNfe", async (req, res) => {
  ...
  try {
    ...
    let url = `${NfeConfig.url}/v2/nfce/${referencia}`;
    ...
    return res
      .status(400)
      .send({ message: "Ocorreu um erro ao cancelar a NFCE" });
  }
});
...
router.post("/nfeRascDev", async (req, res) => {
  ...
  try {
    ...
    const rasc = await NFCE.findOneAndUpdate(
      { sale: venda },
      {
        $set: {
    ...
    return res
      .status(400)
      .send({ message: "Erro ao gerar o rascunho da NFCE" });
  }
});
...
router.post("/sendDevNfe", async (req, res) => {
  ...
  try {
    const rasc = await NFCE.findOne({ sale: venda });

    let urlNfe = `${NfeConfig.url}/v2/nfce?ref=${rasc._id}`;
    ...
    if (statusResponse === "processando_autorizacao") {
      ...
      await Vendas.findByIdAndUpdate(rasc.sale, {
        $set: { nfceStatus: info, devolve: true },
      });
      return res.status(200).send({
        message:
          "Aguarde alguns instantes, a NFCE está sendo processada pela SEFAZ",
...
router.post("/cartaCorrect", async (req, res) => {
  ...
  try {
    let urlNfe = `${NfeConfig.url}/v2/nfce/${ref}/carta_correcao`;
...
router.post("/sendEmailNfe", async (req, res) => {
  ...
  try {
    let url = `${NfeConfig.url}/v2/nfce/${ref}/email`;
...
router.get("/findRasc/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const rasc = await NFCE.findOne({ sale: id });
    const sale = await Vendas.findOne({ _id: id });
    return res.status(200).send({ rasc, sale });
  } catch (error) {
    return res
      .status(400)
      .send({ message: "Erro ao buscar o Rascunho da NFCE" });
  }
});

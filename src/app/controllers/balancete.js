...
router.get('/list', async (req, res) => {
    ...
    try {
        ...
        const balanco = await Balancete.find({ anual: false }).sort({ dateSave: 1 });
...
router.get('/lastGer', async (req, res) => {
    ...
    try {
        ...
        const balanco = await Balancete.find({ anual: false }).sort({ $natural: -1 }).limit(1);
...
router.get('/lastGerAnual', async (req, res) => {
    ...
    try {
        ...
        const balanco = await Balancete.find({ anual: true }).sort({ $natural: -1 }).limit(1);

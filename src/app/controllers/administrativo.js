...
router.post('/auth', async (req, res) => {
    ...
    try {
        ...
        const funcionario = await Funcionario.findOne({ user: user }).select('user password admin sales caixa name active');
...
router.get('/findFuncionariosComissioned', async (req, res) => {
    ...
    try {
        ...
        const funcionarios = await Funcionario.find({ comissioned: true, active: true }).sort({ name: 1 });
...
router.put('/changeFuncStatus/:id', async (req, res) => {
    ...
    const { id } = req.params;
    const { admin, sales, caixa, comission, user, password, cargo, comissioned } = req.body;
...
router.put('/editCliente/:id', async (req, res) => {
    ...
    const { id } = req.params;
    const { name, cpf_cnpj, rg, emitter, dateBirth, email, socialName, stateRegistration, municipalRegistration, manager, phoneComercial, celOne, celTwo, obs } = req.body;
...
router.post('/buscaGestao', async (req, res) => {
    ...
    try {
        ...
        if (find === 3) {
            ...
            const clientes = await Clientes.find({ restrict: true }).sort({ name: 1 });
...
router.get('/findProducts', async (req, res) => {
    ...
    try {
        ...
        const products = await Produtos.find().sort({ codiname: 1 }).populate({ path: 'fornecedor', select: 'name' });
...
router.get('/listProductsChanged', async (req, res) => {
    try {
        const produtos = await Produtos.find({ confirmStatus: true }).sort({ codiname: 1 }).populate({ path: 'fornecedor', select: 'name' });
...
router.put('/changeInfoProduct/:id', async (req, res) => {
    ...
    const { id } = req.params;
    const { codiname, code, codeUniversal, unMedida, description, cfop, ncm, cest, valueCusto, valueDiversos, valueSale, estoqueAct, icms, pis, cofins, margeLucro, markupFactor, modeCalc } = req.body;
    ...
    try {
        ...
        await Produtos.findByIdAndUpdate(id, {
            $set: {
                codiname, code, codeUniversal, unMedida, description, cfop, ncm, cest, valueCusto, valueDiversos, valueSale, estoqueAct, icms, pis, cofins, confirmStatus: false, createDate: dataCadastro, margeLucro: margeLucro, atualized: false, markupFactor, typeCalculate: modeCalc
            }
        });
...
router.post('/listDebitsAll', async (req, res) => {
    ...
    try {
        ...
        if (find === 1) {
            ...
            const produtos = await PagamentoVendas.find({ statusPay: 'wait', dateToPay: { $lte: Data } }).populate({ path: 'cliente', select: 'name' }).sort({ dateToPay: 1 });

            const servicos = await PagamentoServicos.find({ statusPay: 'wait', dateToPay: { $lte: Data } }).populate({ path: 'cliente', select: 'name' }).sort({ dateToPay: 1 });
        ...
        if (find === 2) {
            ...
            const produtos = await PagamentoVendas.find({ cliente: client, statusPay: 'wait', dateToPay: { $lte: Data } }).populate({ path: 'cliente', select: 'name' }).sort({ dateToPay: 1 });

            const servicos = await PagamentoServicos.find({ cliente: client, statusPay: 'wait', dateToPay: { $lte: Data } }).populate({ path: 'cliente', select: 'name' }).sort({ dateToPay: 1 });
...
router.post('/findAllSalesAndOrders', async (req, res) => {
    ...
    try {
        ...
        if(find === 1) {
            ...
            const vendas = await Vendas.find({ statuSales: 'sale', waiting: 'none', finish: 'yes', month: meses[monthDate], year: yearDate }).populate({ path: 'planoDeConta funcionario client address', select: 'name planoConta street number bairro city state phoneComercial celOne' });

            const ordens = await Ordens.find({ statuSales: 'sale', waiting: 'none', finish: 'yes', month: meses[monthDate], year: yearDate }).populate({ path: 'planoDeConta funcionario client veicles', select: 'name planoConta model street number bairro city state phoneComercial celOne' });
        ...
        if(find === 2) {
            ...
            const vendas = await Vendas.find({ statuSales: 'sale', waiting: 'none', finish: 'yes', month: mes, year: ano }).populate({ path: 'planoDeConta funcionario client address', select: 'name planoConta street number bairro city state phoneComercial celOne' });

            const ordens = await Ordens.find({ statuSales: 'sale', waiting: 'none', finish: 'yes', month: mes, year: ano }).populate({ path: 'planoDeConta funcionario client veicles', select: 'name planoConta model street number bairro city state phoneComercial celOne' });
        ...
        if (find === 3) {
            ...
            const vendas = await Vendas.find({ client: client, statuSales: 'sale', waiting: 'none', finish: 'yes' }).populate({ path: 'planoDeConta funcionario client address', select: 'name planoConta street number bairro city state phoneComercial celOne' });

            const ordens = await Ordens.find({ client: client, statuSales: 'sale', waiting: 'none', finish: 'yes' }).populate({ path: 'planoDeConta funcionario client veicles address', select: 'name planoConta model street number bairro city state phoneComercial celOne' });
        ...
        if(find === 4) {
            ...
            const vendas = await Vendas.find({ number: number, statuSales: 'sale', waiting: 'none', finish: 'yes' }).populate({ path: 'planoDeConta funcionario client address', select: 'name planoConta street number bairro city state phoneComercial celOne' });

            const ordens = await Ordens.find({ number: number, statuSales: 'sale', waiting: 'none', finish: 'yes' }).populate({ path: 'planoDeConta funcionario client veicles', select: 'name planoConta model street number bairro city state phoneComercial celOne' });

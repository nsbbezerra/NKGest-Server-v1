...
router.post("/open", async (req, res) => {
  ...
  if (findFuncionario.caixa !== true && findFuncionario.admin !== true) {
    return res
      .status(400)
      .send({ message: "Funcionário sem autorização para efetuar esta ação" });
  }
...
router.delete("/delMoviment/:id", async (req, res) => {
  ...
  const findPerCaixa = await Movimentacao.findOne({ _id: id }).select(
    "idCaixa typeMoviment value"
  );

  const findCaixa = await Caixa.findOne({ _id: findPerCaixa.idCaixa }).select(
    "saldo"
  );
...
router.put("/close/:id", async (req, res) => {
  ...
  try {
    const findValueOpened = await Caixa.findOne({ _id: id }).select(
      "valueOpened"
    );
    const findCredits = await Movimentacao.find({
      idCaixa: id,
      typeMoviment: "credit",
    }).select("value");
    const findDebits = await Movimentacao.find({
      idCaixa: id,
      typeMoviment: "debit",
    }).select("value");
    ...
    return res
      .status(200)
      .send({ valueOpenedAct, calcCredits, calcDebits, valueFechamento });
  } catch (error) {
...
router.post("/findMoviment/:id", async (req, res) => {
  ...
  try {
    const findCaixa = await Caixa.findOne({ _id: id }).populate({
      path: "funcionario",
      select: "name",
    });
    ...
    return res
      .status(400)
      .send({ message: "Erro ao buscar informações sobre o caixa" });
  }
});
...
router.post("/authenticate/:id", async (req, res) => {
  ...
  const funcionario = await Funcionario.findOne({ _id: id }).select(
    "user password"
  );
...
router.get("/listOpen", async (req, res) => {
  try {
    const caixas = await Caixa.find({ status: "open" })
      .populate({ path: "funcionario", select: "name" })
      .sort({ $natural: -1 });
...
router.get("/listClose", async (req, res) => {
  try {
    const caixas = await Caixa.find({ status: "close" })
      .populate({ path: "funcionario", select: "name" })
      .sort({ $natural: -1 });
...
router.post("/advancedFind", async (req, res) => {
  ...
  try {
    ...
    if (type === 3) {
      const caixa = await Caixa.find({ status: "close" }).populate({
        path: "funcionario",
        select: "name",
      });
...
router.put("/closeService/:id", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send({ message: "Ordem de serviço encerrada com sucesso" });
  } catch (error) {
    return res
      .status(400)
      .send({ message: "Erro ao encerrar a ordem de serviço" });
  }
});
  ...
  try {
    const sales = await Venda.find({
      ...
      .populate({
        ...
        select: "name street number bairro city state",
      })
      .sort({ dateSave: -1 });
    const services = await Servicos.find({
      ...
      .populate({
        ...
        select: "name model street number bairro city state",
      })
      .sort({ dateSave: -1 });
...
router.get("/findCashierOrders/:id", async (req, res) => {
  ...
  try {
    const vendas = await Venda.find({
      ...
      .populate({ path: "client funcionario", select: "name" })
      .sort({ dateSave: 1 });
